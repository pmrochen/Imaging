...
struct Rectangle<T>
{
	...
	template<Arithmetic U> explicit Rectangle(const Rectangle<U>& rect) noexcept : x(T(rect.x)), y(T(rect.y)), width(T(rect.width)), height(T(rect.height)) {}

	bool operator==(const Rectangle& rect) const noexcept { return (x == rect.x) && (y == rect.y) && (width == rect.width) && (height == rect.height); }
	...
	bool approxEquals(const Rectangle& rect) const noexcept;
	bool approxEquals(const Rectangle& rect, T tolerance) const noexcept;
	bool isFinite() const noexcept { return std::isfinite(x) && std::isfinite(y) && std::isfinite(width) && std::isfinite(height); }
	...
	template<std::floating_point U = T> getAspectRatio() const noexcept { return (height != T(0)) ? U((double)width/(double)height) : U(); }
	Rectangle& inflate(T size) noexcept { x -= size; y -= size; this->width += T(2)*size; this->height += T(2)*size; return *this; }
	Rectangle& inflate(T width, T height) noexcept { x -= width; y -= height; this->width += T(2)*width; this->height += T(2)*height; return *this; }
	...
	static Rectangle makeUnion(const Rectangle& a, const Rectangle& b) noexcept { return Rectangle(Uninitialized()).setUnion(a, b); }
	static Rectangle makeIntersection(const Rectangle& a, const Rectangle& b) noexcept { return Rectangle(Uninitialized()).setIntersection(a, b); }
...
template<std::floating_point T> const Rectangle<T> Rectangle<T>::EMPTY{};
...
struct Rectangle<T>
{
	...
	Rectangle& set(T x, T y, T width, T height) noexcept;
	Rectangle& setLeftTopRightBottom(T left, T top, T right, T bottom) noexcept;
	template<std::floating_point U = float> getAspectRatio() const noexcept { return (height != T(0)) ? U((double)width/(double)height) : U(); }
	Rectangle& inflate(T size) noexcept { x -= size; y -= size; this->width += T(2)*size; this->height += T(2)*size; return *this; }
	Rectangle& inflate(T width, T height) noexcept { x -= width; y -= height; this->width += T(2)*width; this->height += T(2)*height; return *this; }
	Rectangle& translate(T x, T y) noexcept { this->x += x; this->y += y; return *this; }
	Rectangle& setUnion(const Rectangle& a, const Rectangle& b) noexcept;
...
template<std::integral T> const Rectangle<T> Rectangle<T>::EMPTY{};
...
template<std::floating_point T>
inline bool Rectangle<T>::isApproxZero() const
{ 
...
template<std::floating_point T>
inline bool Rectangle<T>::approxEquals(const Rectangle<T>& rect) const
{ 
...
template<std::floating_point T>
inline bool Rectangle<T>::approxEquals(const Rectangle<T>& rect, T tolerance) const
{ 
