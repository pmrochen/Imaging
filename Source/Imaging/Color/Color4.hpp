...
struct Color4<T>
{
	...
	explicit Color4(const Tuple4<T>& t) noexcept : r(t.x), g(t.y), b(t.z), a(t.w) {}
	template<Arithmetic U> explicit Color4(const Tuple4<U>& t) noexcept : r(T(t.x)), g(T(t.y)), b(T(t.z)), a(T(t.w)) {}
	explicit Color4(const TupleType& t) noexcept : r(std::get<0>(t)), g(std::get<1>(t)), b(std::get<2>(t)), a(std::get<3>(t)) {}
	template<Arithmetic U> explicit Color4(const OtherTupleType<U>& t) noexcept : r(T(std::get<0>(t))), g(T(std::get<1>(t))), b(T(std::get<2>(t))), a(T(std::get<3>(t))) {}
	...
	operator Tuple4<T>() const noexcept { return Tuple4<T>(r, g, b, a); }
	template<Arithmetic U> explicit operator Tuple4<U>() const noexcept { return Tuple4<U>(U(r), U(g), U(b), U(a)); }
	operator TupleType() const noexcept { return TupleType(r, g, b, a); }
	template<Arithmetic U> explicit operator OtherTupleType<U>() const noexcept { return OtherTupleType<U>(U(r), U(g), U(b), U(a)); }
	...
	const Color3<T>& rgb/*getRgb*/() const noexcept { return reinterpret_cast<const Color3<T>&>(*this); }
...
template<>
struct alignas(16) Color4<float>
{
	using Real = float;
	...
	explicit Color4(const Tuple4<float>& t) noexcept : rgba(simd::set4(t.x, t.y, t.z, t.w)) {}
	template<Arithmetic U> explicit Color4(const Tuple4<U>& t) noexcept : rgba(simd::set4((float)t.x, (float)t.y, (float)t.z, (float)t.w)) {}
	explicit Color4(const TupleType& t) noexcept : rgba(simd::set4(std::get<0>(t), std::get<1>(t), std::get<2>(t), std::get<3>(t))) {}
	template<Arithmetic U> explicit Color4(const OtherTupleType<U>& t) noexcept : rgba(simd::set4((float)std::get<0>(t), (float)std::get<1>(t), (float)std::get<2>(t), (float)std::get<3>(t))) {}
	...
	operator Tuple4<float>() const noexcept { return Tuple4<float>(r, g, b, a); }
	template<Arithmetic U> explicit operator Tuple4<U>() const noexcept { return Tuple4<U>(U(r), U(g), U(b), U(a)); }
	operator TupleType() const noexcept { return TupleType(r, g, b, a); }
	template<Arithmetic U> explicit operator OtherTupleType<U>() const noexcept { return OtherTupleType<U>(U(r), U(g), U(b), U(a)); }
...
inline T& Color4<T>::get() noexcept
{
	if constexpr (I == 0)
		return r;
...
template<std::floating_point T>
template<std::size_t I>
inline const T& Color4<T>::get() const noexcept
{
	if constexpr (I == 0)
		return r;
	else if constexpr (I == 1)
...
template<std::floating_point T>
template<typename U> 
inline Color4<T> Color4<T>::fromPackedRgba(U c) noexcept
{
	Color4<T> result(unpackRgba<Color4<T>>(c));
	result *= T(1)/(T)pixel::PackedRgba<U>::R_MAX;
	return result;
}

template<std::floating_point T>
template<typename U> 
inline U Color4<T>::toPackedRgba() const noexcept
{
	return makePackedRgba<U>(saturate<int, pixel::PackedRgba<U>::R_MAX>(r),
		saturate<int, pixel::PackedRgba<U>::G_MAX>(g),
		saturate<int, pixel::PackedRgba<U>::B_MAX>(b),
		saturate<int, pixel::PackedRgba<U>::A_MAX>(a));
}

template<std::floating_point T>
template<typename U> 
inline Color4<T> Color4<T>::fromPackedBgra(U c) noexcept
{
	Color4<T> result(unpackBgra<Color4<T>>(c));
	result *= T(1)/(T)pixel::PackedBgra<U>::R_MAX;
	return result;
}

template<std::floating_point T>
template<typename U> 
inline U Color4<T>::toPackedBgra() const noexcept
{
	return makePackedBgra<U>(saturate<int, pixel::PackedBgra<U>::R_MAX>(r),
		saturate<int, pixel::PackedBgra<U>::G_MAX>(g),
		saturate<int, pixel::PackedBgra<U>::B_MAX>(b),
		saturate<int, pixel::PackedBgra<U>::A_MAX>(a));
}

template<std::floating_point T>
inline bool Color4<T>::isApproxZero() const noexcept
{ 
...
template<std::floating_point T>
inline bool Color4<T>::approxEquals(const Color4<T>& c) const noexcept
{ 
...
template<std::floating_point T>
inline bool Color4<T>::approxEquals(const Color4<T>& c, T tolerance) const noexcept
{ 
...
template<std::floating_point T>
inline Color4<T>& Color4<T>::makeLinear() noexcept
{ 
	r = makeLinear(r);
	g = makeLinear(g);
	b = makeLinear(b);
...
template<std::floating_point T>
inline Color4<T>& Color4<T>::makeSrgb() noexcept
{ 
	r = makeSrgb(r);
	g = makeSrgb(g);
	b = makeSrgb(b);
...
template<std::size_t I>
inline float& Color4<float>::get() noexcept
{
	if constexpr (I == 0)
		return r;
	else if constexpr (I == 1)
...
template<std::size_t I>
inline const float& Color4<float>::get() const noexcept
{
	if constexpr (I == 0)
		return r;
	else if constexpr (I == 1)
...
template<typename U> 
inline U Color4<float>::toPackedRgba() const noexcept
{
...
template<typename U> 
inline U Color4<float>::toPackedBgra() const noexcept
{
...
inline float luminance(const Color4<float>& c) noexcept
{
	...
	return simd::extract(simd::dot3(c, Color4<float>::LUMINANCE/*coeff*/));
}
...
struct tuple_element<I, ::imaging::templates::Color4<T>>
{
	using type = T;
};

template<typename T>
struct tuple_size<::imaging::templates::Color4<T>> : public integral_constant<size_t, 4> 
{
};

template<typename T>
struct hash<::imaging::templates::Color4<T>>
{
	size_t operator()(const ::imaging::templates::Color4<T>& c) const noexcept
	{
		hash<T> hasher;
...
template<>
struct hash<::imaging::templates::Color4<float>>
{
	size_t operator()(const ::imaging::templates::Color4<float>& c) const noexcept
	{
		return hash<typename ::simd::float4>()(c.rgba);
	}
};
